"""Configuration for the Libero living room scene6 put the white mug on the plate task."""
...
@register_task(
    "libero_90.living_room_scene6_put_the_white_mug_on_the_plate",
    "living_room_scene6_put_the_white_mug_on_the_plate",
)
class LiberoLivingRoomScene6PutWhiteMugOnThePlateTask(Libero90BaseTask):
    """Task: Put the porcelain_mug on the plate in living_room_scene6.
    ...
    Goal: Place porcelain_mug on the plate.
    """
    ...
    task_desc = "Put the porcelain_mug on the plate (living_room_scene6)"

    workspace_name = ("living_room_table",)
    workspace_offset = ((0.0, 0, 0.42),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_living_room_scene6_put_the_white_mug_on_the_plate_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker."""
        white_mug_pos = states.objects["porcelain_mug"].root_state[:, :3]  # (N,3)
        plate_pos = states.objects["plate"].root_state[:, :3]  # (N,3)
        range_threshold = 0.06
        height_threshold = 0.03
        xy_distance = torch.norm(white_mug_pos[:, :2] - plate_pos[:, :2], dim=-1)
        height_diff = white_mug_pos[:, 2] - plate_pos[:, 2]
