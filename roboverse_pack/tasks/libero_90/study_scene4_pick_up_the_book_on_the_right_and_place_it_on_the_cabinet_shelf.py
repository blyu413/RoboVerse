"""Configuration for the Libero study scene4 pick up the book on the right and place it on the cabinet shelf task."""
...
class LiberoStudyScene4PickUpTheBookOnTheRightAndPlaceItOnTheCabinetShelfTask(Libero90BaseTask):
    """Configuration for the Libero study scene4 pick up the book on the right and place it on the cabinet shelf task.

    Task Description:
    - Pick up the yellow_book_1 (right)
    - Place it on the wooden_two_layer_shelf (top region)
    ...
    Goal: Place yellow_book_1 on wooden_two_layer_shelf (top region).
    """
    ...
    task_desc = "Pick up the yellow_book_1 on the right and place it on the top region of the wooden_two_layer_shelf (study_scene4)"

    workspace_name = ("study_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_study_scene4_pick_up_the_book_on_the_right_and_place_it_on_the_cabinet_shelf_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: yellow_book_1 is in the top region of the wooden_two_layer_shelf.

        Success condition:
        - yellow_book_1 is within the bounding box region defined by the top_region site
        ...
        # top_region site half-size from wooden_shelf.xml: size="0.03272 0.05000 0.11027"
        ...
        # Transform book position to shelf top_region local frame
