"""Configuration for the Libero study scene3 pick up the book and place it in the left compartment of the caddy task."""
...
class LiberoStudyScene3PickUpTheBookAndPlaceInTheLeftCompartmentOfTheCaddyTask(Libero90BaseTask):
    """Configuration for the Libero study scene3 pick up the book and place it in the left compartment of the caddy task.

    Task Description:
    - Pick up the black_book from the table
    - Place the black_book inside the desk_caddy/left_contain_region

    This is a manipulation task that requires:
    1. Picking up the black_book from the table
    2. Placing the black_book inside the desk_caddy/left_contain_region
    ...
    Goal: Place black_book inside desk_caddy/left_contain_region.
    """
    ...
    task_desc = "Pick up the black_book and place it in the left compartment of the desk_caddy (study_scene3)"

    workspace_name = ("study_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_study_scene3_pick_up_the_book_and_place_it_in_the_left_compartment_of_the_caddy_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: black_book is inside desk_caddy/left_contain_region bounding box."""
        book_pos = states.objects["black_book"].root_state[:, :3]
        N = book_pos.shape[0]
        region_mat = self.handler.physics.named.data.site_xmat["desk_caddy/left_contain_region"]
        region_pos = self.handler.physics.named.data.site_xpos["desk_caddy/left_contain_region"]
