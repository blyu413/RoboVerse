"""Configuration for the Libero study scene4 pick up the book on the left and place it on top of the shelf task."""
...
class LiberoStudyScene4PickUpTheBookOnTheLeftAndPlaceItOnTopOfTheShelfTask(Libero90BaseTask):
    """Configuration for the Libero study scene4 pick up the book on the left and place it on top of the shelf task.

    Task Description:
    - Pick up the yellow_book_2 (left)
    - Place it on the wooden_two_layer_shelf (top side)

    Objects:
    - black_book
    - yellow_book_1
    - yellow_book_2 (target)
    - wooden_two_layer_shelf (goal shelf)

    Goal: Place yellow_book_2 on wooden_two_layer_shelf (top side).
    """
    ...
    task_desc = "Pick up the yellow_book_2 on the left and place it on top of the wooden_two_layer_shelf (study_scene4)"

    workspace_name = ("study_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_study_scene4_pick_up_the_book_on_the_left_and_place_it_on_top_of_the_shelf_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: yellow_book_2 is on the top side of the wooden_two_layer_shelf.

        Success condition:
        - yellow_book_2 is within the bounding box region defined by the top_side site
        - Uses the site's position and orientation from physics data
        """
        book_pos = states.objects["yellow_book_2"].root_state[:, :3]  # (N,3)
        N = book_pos.shape[0]
