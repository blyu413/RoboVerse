"""Configuration for the Libero living room scene5 put the yellow and white mug on the right plate task."""
...
class LiberoLivingRoomScene5PutTheYellowAndWhiteMugOnTheRightPlateTask(Libero90BaseTask):
    """Configuration for the Libero living room scene5 put the yellow and white mug on the right plate task.
    ...
    Goal: white_yellow_mug is on plate_2.
    """
    ...
    task_desc = "Put the yellow and white mug on the right plate (living_room_scene5)"

    workspace_name = ("living_room_table",)
    workspace_offset = ((0.0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_living_room_scene5_put_the_yellow_and_white_mug_on_the_right_plate_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker."""
        mug_pos = states.objects["white_yellow_mug"].root_state[:, :3]  # (N,3)
        plate_pos = states.objects["plate_2"].root_state[:, :3]  # (N,3)
        range_threshold = 0.06
        height_threshold = 0.03
        xy_distance = torch.norm(mug_pos[:, :2] - plate_pos[:, :2], dim=-1)
        height_diff = mug_pos[:, 2] - plate_pos[:, 2]
