"""Configuration for the Libero study scene3 pick up the red mug and place it to the right of the caddy task."""
...
class LiberoStudyScene3PickUpTheRedMugAndPlaceItToTheRightOfCaddyTask(Libero90BaseTask):
    """Configuration for the Libero study scene3 pick up the red mug and place it to the right of the caddy task.

    Task Description:
    - Pick up the red_coffee_mug from the table
    - Place the red_coffee_mug inside the desk_caddy/right_contain_region

    This is a manipulation task that requires:
    1. Picking up the red_coffee_mug from the table
    2. Placing the red_coffee_mug inside the desk_caddy/right_contain_region

    Objects:
    - black_book
    - red_coffee_mug (target)
    - porcelain_mug
    - desk_caddy (goal container)

    Goal: Place red_coffee_mug inside desk_caddy/right_contain_region.
    """
    ...
    task_desc = "Pick up the red_coffee_mug and place it to the right of the desk_caddy (study_scene3)"

    workspace_name = ("study_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_study_scene3_pick_up_the_red_mug_and_place_it_to_the_right_of_the_caddy_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: red_coffee_mug is to the right of desk_caddy (x > caddy.x + threshold, y/z close)."""
        mug_pos = states.objects["red_coffee_mug"].root_state[:, :3]
