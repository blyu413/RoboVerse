"""Configuration for the Libero study scene1 pick up the book and place it in the front compartment of the caddy task."""
...
class LiberoStudyScene1PickUpTheBookAndPlaceInTheFrontCompartmentOfTheCaddyTask(Libero90BaseTask):
    """Configuration for the Libero study scene1 pick up the book and place it in the front compartment of the caddy task.

    Task Description:
    - Pick up the black_book from the table
    - Place the black_book inside the desk_caddy/front_contain_region

    This is a manipulation task that requires:
    1. Picking up the black_book from the table
    2. Placing the black_book inside the desk_caddy/front_contain_region
    ...
    task_desc = "Pick up the black_book and place it in the front containment region of the desk_caddy (study_scene1)"

    workspace_name = ("study_table",)
    workspace_offset = ((0, 0, 0),)
    workspace_size = ((1.0, 1.2, 0.1),)

    traj_filepath = "roboverse_data/trajs/libero90/libero_90_study_scene1_pick_up_the_book_and_place_it_in_the_front_compartment_of_the_caddy_traj_v2.pkl"

    def _terminated(self, states: TensorState) -> torch.Tensor:
        """Task success checker: black_book is inside desk_caddy/front_compartment bounding box."""
        book_pos = states.objects["black_book"].root_state[:, :3]
        N = book_pos.shape[0]
        region_mat = self.handler.physics.named.data.site_xmat["desk_caddy/front_contain_region"]
        region_pos = self.handler.physics.named.data.site_xpos["desk_caddy/front_contain_region"]
        ...
        # The half_size should be set according to the actual compartment size; placeholder values below
        half_size = torch.tensor([0.02775, 0.03595, 0.06216], device=book_pos.device)
